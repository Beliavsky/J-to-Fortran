...
"""
    ...
    assert generated.splitlines()[:3] == [
        f"! Generated by xj2f.py {xj2f.VERSION} from comments.ijs",
        "! Sum the zero-based integers below y.",
        "",
    ]
    assert "! J: sumto =: 3 : 0" not in generated
    assert (
...
"""
    ...
    assert "! J: value =: 1 + 2" not in commented
    assert "! J: echo value" not in commented
    assert "! J: value =: 1 + 2" in all_comments
    assert "! J: echo value" in all_comments
    assert "! Compute a value." not in no_comments
    assert "! J:" not in no_comments


def test_black_scholes_leading_comment_is_a_file_header() -> None:
    generated = xj2f.transpile_path(EXAMPLES / "black_scholes.ijs")

    assert generated.splitlines()[:3] == [
        f"! Generated by xj2f.py {xj2f.VERSION} from black_scholes.ijs",
